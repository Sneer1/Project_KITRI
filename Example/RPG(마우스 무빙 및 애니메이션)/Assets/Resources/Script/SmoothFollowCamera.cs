...
public class SmoothFollowCamera : MonoBehaviour {
	...
	void LateUpdate()
	{
		...
		if (Input.GetAxis ("Mouse ScrollWheel") != 0.0f) {
			...
			newheight = height + -Input.GetAxis ("Mouse ScrollWheel") * changeSpeedForDistance/2.0f;
		}
		
		if (newDistance != distance) {
			distance = Mathf.SmoothDamp (distance, newDistance, ref zoomVelocity, targetChangeTime);
			height = Mathf.SmoothDamp (height, newheight, ref zoomVelocity, targetChangeTime);
		}
