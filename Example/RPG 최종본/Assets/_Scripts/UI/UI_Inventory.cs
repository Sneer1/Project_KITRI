...
public class UI_Inventory : BaseObject
{
	...
	public void Init()
	{
		...
		EventDelegate.Add(CloseBtn.onClick,
			new EventDelegate(this, "HideInventory"));

		WeaponLabel = FindInChild("Weapon").
			Find("ItemName").GetComponent<UILabel>();

		HelmatLabel = FindInChild("Helmat").
			Find("ItemName").GetComponent<UILabel>();

		GuntletLabel = FindInChild("Guntlet").
			Find("ItemName").GetComponent<UILabel>();

		ArmorLabel = FindInChild("Armor").
			Find("ItemName").GetComponent<UILabel>();
		...
		foreach(KeyValuePair<eSlotType,ItemInstance> pair in dic)
		{
			switch (pair.Key)
			{
				...
				case eSlotType.SLOT_HELMET:
					HelmatLabel.text = pair.Value.ITEM_INFO.NAME;
					break;
