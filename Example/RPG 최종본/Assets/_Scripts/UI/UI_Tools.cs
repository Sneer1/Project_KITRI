...
public class UI_Tools : MonoSingleton<UI_Tools>
{
	...
	GameObject GetUI(eUIType _uiType, bool isDontDestroy)
	{
		...
		GameObject prefabUI =
			Resources.Load("Prefabs/UI/" +
			_uiType.ToString()) as GameObject;
		...
		if(prefabUI != null)
		{
			...
			{
				// UI_Tools-(Child)-SubRoot
				// SubRoot Child
				if(SubRoot == null)
				{
					GameObject root =
						Resources.Load("Prefabs/UI/SubUI_Root")
						as GameObject;

					// this :: UITools : MonoSingleton ( DontDestroy )
					SubRoot = NGUITools.AddChild(
